...
from problems.two_pointers import threesum, two_sum_2, merge_sorted_array
...
class TestMergeSortedArray(unittest.TestCase):
    def test_merge_sorted_array(self):
        ...
        test_cases = [
            ...
            TestCase(
                name="example 3",
                inputs=Params([0], 0, [1], 1),
                want=[1]
            ),
            TestCase(
                name="example 4",
                inputs=Params([1,0], 1, [2], 1),
                want=[1,2]
            ),
            TestCase(
                name="example 5",
                inputs=Params([2,0], 1, [1], 1),
                want=[1,2]
            )
        ]
        for case in test_cases:
            got = case.inputs.nums1
            merge_sorted_array(case.inputs.nums1, case.inputs.m, case.inputs.nums2, case.inputs.n)
            self.assertEqual(
                got,
                case.want,
                f"\nFailed test '{case.name}'. got: '{got}', want: '{case.want}'.",
